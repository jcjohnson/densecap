...
A DetectionModule wraps up all of the complexities of detection regions and
...
of the underlying image for that forward pass.
...
local layer, parent = torch.class('nn.DetectionModule', 'nn.Module')
...
function layer:forward_test(cnn_features, arg)
  arg = arg or {}
  arg.clip_boxes = utils.getopt(arg, 'clip_boxes')
  arg.nms_thresh = utils.getopt(arg, 'nms_thresh')
  arg.max_proposals = utils.getopt(arg, 'max_proposals', -1)
...
function layer:updateOutput(input)
  local cnn_features, gt_boxes, gt_labels = unpack(input)
...
function layer:updateGradInput(input, gradOutput)
  ...
  self.gradInput[1]:resizeAs(cnn_features):zero()
  local grad_cnn_features = self.gradInput[1]
  ...
  self:timeit('rpn:backward', function()
    ...
    -- self.gradInput[1]:add(grad_cnn_features)
  end)
...
function build_rpn(opt)
  -- Replace the string backend with an object
  local backend = nil
  ...
  local conv_layer = backend.SpatialConvolution(
                        opt.input_dim,
                        opt.rpn_num_filters,
                        opt.rpn_filter_size,
                        opt.rpn_filter_size,
  ...
  local box_conv_layer = backend.SpatialConvolution(
                            opt.rpn_num_filters,
                            4 * num_anchors,
                            1, 1, 1, 1, 0, 0)
  ...
  local rpn_conv_layer = backend.SpatialConvolution(
