...
def encode_boxes(data, original_heights, original_widths, image_size):
  ...
  for i, img in enumerate(data):
    ...
    for region in img['regions']:
      ...
      w, h = round(scale*region['w']), round(scale*region['h'])  
  ...
  for i, img in enumerate(data):
    ...
    for region in img['regions']:
      ...
      if tokens_type == 'words':
        tokens = words_preprocess(region['label'])
...
def encode_splits(data):
  ...
  conv = {'train':0, 'val':1, 'test':2}
  for i, img in enumerate(data):
    split[i] = conv[img['split']] # encode splits as indices, 0=train, 1=val, 2=test
...
if __name__ == '__main__':
  ...
  parser.add_argument('--min_token_instances', default=15, type=int, help="when token appears less than this times it will be mapped to <UNK>")
  parser.add_argument('--tokens_type', default='words', help="words|chars for word or char split in captions")
